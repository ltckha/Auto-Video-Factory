...
4. **`subtitle_style`**: Standard styles (`vibrant_yellow_sticker`, `vibrant_yellow_lightning_sticker`, `minimal_glass_card`, `warning_red_badge`, `hook_bold`, `gold_caption`, `framed_card`, `cta_red`, `neon_glow`) OR any creative custom style name generated by AI (unmapped creative names will be logged into backlog automatically).
