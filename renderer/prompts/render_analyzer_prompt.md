...
4. **`subtitle_style`**: Enforce single primary style cohesion across body scenes (`vibrant_yellow_sticker`, `minimal_glass_card`, `warning_red_badge`, etc.). Allow `vibrant_yellow_lightning_sticker` or `warning_red_badge` for the final outro scene. Unmapped creative style names will be logged into backlog automatically.
...
- **Mandatory Subtitle Line Break `\n`**: If `subtitle` is longer than 16 characters and does not contain `\n`, you MUST insert `\n` to split it into 2 balanced lines (max 14-16 chars/line).
